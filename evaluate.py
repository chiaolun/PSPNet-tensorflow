...
ADE20k_param = {'crop_size': [473, 473],
                # predict: [0~149] corresponding to label [1~150], ignore class 0 (background)
                'num_classes': 150,
                'ignore_label': 0,
                'num_steps': 2000,
                'model': PSPNet50,
...
def main():
    ...
    shape = tf.shape(img)
    h, w = (tf.maximum(crop_size[0], shape[0]),
            tf.maximum(crop_size[1], shape[1]))
    ...
    raw_output_up = tf.image.resize_bilinear(
        raw_output, size=[h, w], align_corners=True)
    ...
    if args.dataset == 'ade20k':
        ...
        mIoU, update_op = tf.contrib.metrics.streaming_mean_iou(
            pred, gt, num_classes=num_classes+1)
    elif args.dataset == 'cityscapes':
        mIoU, update_op = tf.contrib.metrics.streaming_mean_iou(
    ...
    for step in trange(num_steps, desc='evaluation', leave=True):
        ...
        _ = sess.run(update_op, feed_dict={
                     image_filename: f1, anno_filename: f2})
